...
class RegisterActivity : AppCompatActivity() {
    ...
    private fun areCredentialsGood(): Boolean {
        val emailMatch: Boolean = DataChecker.getInstance().checkData(binding.registerEmailInput.editText?.text.toString(), DataChecker.DATA_TYPE_EMAIL)
        val passwordMatch: Boolean = DataChecker.getInstance().checkData(binding.registerPasswordInput.editText?.text.toString(), DataChecker.DATA_TYPE_PASSWORD)
        val penNameMatch: Boolean = DataChecker.getInstance().checkData(binding.registerPenNameInput.editText?.text.toString(), DataChecker.DATA_TYPE_PEN_NAME)
        var errorMessage = ""
        if(!emailMatch) {
    ...
    private fun register(){
        ...
        val params = JSONObject()
            ...
            .put("password", DataChecker.getInstance().md5(binding.registerPasswordInput.editText?.text.toString()))
            .put("penName", binding.registerPenNameInput.editText?.text.toString())
        val req: JsonObjectRequest = JsonObjectRequest(Request.Method.POST, getString(R.string.endpoint_register), params,
            { response ->
                progress.dismiss()
