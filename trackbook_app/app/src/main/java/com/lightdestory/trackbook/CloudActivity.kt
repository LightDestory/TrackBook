...
class CloudActivity : AppCompatActivity() {
    ...
    private fun requestConfirm(action: String) {
        val titleID: Int =
            if (action == "backup") R.string.endpoint_backup_title else R.string.endpoint_restore_title
        val messageID: Int =
            if (action == "backup") R.string.cloud_backup_desc else R.string.cloud_restore_desc
        ...
        val req: JsonObjectRequest = object : JsonObjectRequest(
            ...
            { response ->
                ...
                if (response.getString("result").contains("[")) {
                    val gson: Gson = Gson()
                    Library.instance.loadBooks(
                        this,
                        gson.fromJson(response.getString("result"), Array<BookReading>::class.java)
                            .toList()
                    )
